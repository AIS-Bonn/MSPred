...
def mask_to_fnums(mask, n_preds, n_seed):
    """ Converting a binary mask into an array of indices """
    range_vect = torch.Tensor(range(n_seed, n_seed + len(mask)))
    fnums = range_vect[mask][:n_preds].numpy().astype(int)
    return fnums


def read_seq(path):
    """
    Reading and concatenating all images in a path
    ...
    --------
    seq: list
        List of numpy arrays. [array1(H, W, 3), ...]
    """
    seq = []
    img_names = sorted(os.listdir(path))
    img_names = [img for img in img_names if img[-4:] in [".png", ".jpg", "jpeg"]]
